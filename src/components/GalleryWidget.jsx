...
export default function GalleryWidget() {
  ...
  const prevSlide = () => {
    ...
    else setCurrent(images.length - 3);
  };

  return (
    <section className="w-full flex items-start gap-3 md:gap-5 rounded-2xl md:p-4 p-2 shadow-neo bg-[#363c43] border border-black/40 overflow-hidden">
      {/* Left side */}
      <div className="flex flex-col gap-20">
        ...
        <div className="grid grid-cols-2 gap-[2px]">
          {Array(6)
            .fill()
            .map((_, i) => (
              <div key={i} className="w-3 h-3 bg-[#4A4E54]"></div>
            ))}
        </div>
      </div>
        ...
        <div className="flex w-full items-center gap-2 md:justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-6">
            ...
            <button
              onClick={prevSlide}
              aria-label="left"
              className="md:w-15 md:h-15 md:text-2xl w-8 h-8 rounded-full bg-gradient-to-r from-zinc-800 to-black/30 hover:bg-black text-zinc-400 hover:text-white transition-colors shadow-[10px_0_15px_rgba(0,0,0,0.41)]"
            >
